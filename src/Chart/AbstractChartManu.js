...
class AbstractChartManu extends Component {
    ...
    return labels.map((label, i) => {
      ...
      return (
        <Text
          ...
          rotation="90"
          originX={((width - paddingRight) / labels.length * (i)) + paddingRight + horizontalOffset}
          originY={(height * 3 / 4) + paddingTop + (fontSize * 2)}
        >{label}
        </Text>
        )
      } else {
  ...
  renderVerticalLines = config => {
    ...
    return [...new Array(data.length)].map((_, i) => {
      return (
        <Line
          key={Math.random()}
          x1={Math.floor((width - paddingRight) / data.length * (4*i) + paddingRight)}
          y1={0}
          x2={Math.floor((width - paddingRight) / data.length * (4*i) + paddingRight)}
...
export default AbstractChartManu
